...
import { CreateTableRow } from './TableRowFactory.js';
/* Appends `count` new rows to the table wrapper, reusing the column count of the first row */
export function AddRows(tableWrapper, tableId, count = 1) {
    ...
    if (existingRowCount > 0) {
        ...
        cellCount =
            Array.from(tableRows[0].children).filter(c => c.classList.contains('table-cell')).length || 1;
    }
...
export function InsertRowBelow(rowElement) {
    ...
    const rows = Array.from(tableWrapper.children);
