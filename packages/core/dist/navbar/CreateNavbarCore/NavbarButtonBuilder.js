import { wireSidebarMenuToggle, wireMenuButtonToggle, } from './NavbarToggleHandlers.js';
/* Creates a single navbar button and attaches any special toggle handlers */
export function buildNavButton(btn) {
    ...
    if (btn.id === 'sidebar-menu')
        wireSidebarMenuToggle(button);
    if (btn.id === 'menu-btn')
        wireMenuButtonToggle(button);
    return button;
}
/* Builds the left container div and appends all left-side buttons */
export function buildLeftContainer(buttons) {
...
export function buildRightContainer(buttons) {
