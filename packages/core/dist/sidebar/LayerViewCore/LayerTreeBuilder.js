...
import { selectLayer, toggleVisibility, deleteComponent, toggleLayerExpansion, } from './LayerItemActions.js';
/* Recursively walks the element's children and appends layer rows to the container */
export function buildLayerTree(element, container, level, refreshFn /* passed down so delete can trigger a full rebuild */) {
    /* Only components tagged as editable or generic canvas components are shown */
    const children = Array.from(element.children).filter(child => child.classList.contains('editable-component') ||
    ...
    children.forEach(child => {
        ...
        const nestedComponents = Array.from(child.children).filter(nested => nested.classList.contains('editable-component') ||
...
function createLayerItem(element, level, refreshFn) {
    ...
    const hasChildren = Array.from(element.children).some(child => child.classList.contains('editable-component') ||
        child.classList.contains('component'));
    layerItem.innerHTML = `
