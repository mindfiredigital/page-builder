...
import { createLocalFileHeader, createCentralDirectoryHeader, createEndOfCentralDirectoryRecord, } from '../../../utils/zipGeneratorCore/zipHeaders.js';
import { stringToUint8Array, assembleZip, } from '../../../utils/zipGeneratorCore/zipAssembler.js';
...
describe('createCentralDirectoryHeader', () => {
    ...
    it('has total length of 46 + fileName.length', () => {
        const header = createCentralDirectoryHeader(fileName, fileContent, fileCrc, localOffset);
    ...
    it('encodes offset 0 correctly', () => {
        const header = createCentralDirectoryHeader(fileName, fileContent, fileCrc, 0);
...
describe('assembleZip', () => {
    it('returns a Blob', () => {
        expect(assembleZip([{ name: 'a.txt', content: 'hello' }])).toBeInstanceOf(Blob);
    });
    it('returns a Blob with MIME type "application/zip"', () => {
        expect(assembleZip([{ name: 'a.txt', content: 'hello' }]).type).toBe('application/zip');
    });
    ...
    it('handles a file with empty content', () => {
        expect(() => assembleZip([{ name: 'empty.txt', content: '' }])).not.toThrow();
    });
    it('handles a file whose name contains path separators', () => {
        expect(() => assembleZip([{ name: 'assets/images/logo.png', content: 'data' }])).not.toThrow();
    });
});
// ===========================================================================
// createZipFile
...
describe('createZipFile', () => {
    it('returns a Blob', () => {
        expect(createZipFile([{ name: 'test.txt', content: 'hi' }])).toBeInstanceOf(Blob);
    });
    it('returns a Blob with MIME type "application/zip"', () => {
        expect(createZipFile([{ name: 'test.txt', content: 'hi' }]).type).toBe('application/zip');
    });
