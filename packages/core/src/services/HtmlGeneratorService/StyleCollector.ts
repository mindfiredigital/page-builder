...
export class StyleCollector {
  ...
  /* ─── IsSVGElement ──────────────────────────────────────────────────────────
     ...
     ─────────────────────────────────────────────────────────────────────────── */
  private isSVGElement(component: Element): boolean {
    return (
      component instanceof SVGElement ||
      (!!component.closest('svg') &&
        SVG_CHILD_TAGS.includes(component.tagName.toLowerCase() as never))
    );
  }
  ...
  private handleSVGElement(
    ...
    const isSVGChild = SVG_CHILD_TAGS.includes(
      component.tagName.toLowerCase() as never
    );
