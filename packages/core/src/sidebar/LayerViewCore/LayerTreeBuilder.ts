import { getComponentType, getComponentName } from './LayerUtils';
import {
  selectLayer,
  toggleVisibility,
  deleteComponent,
  toggleLayerExpansion,
} from './LayerItemActions';
...
export function buildLayerTree(
  ...
  level: number,
  refreshFn: () => void /* passed down so delete can trigger a full rebuild */
): void {
  ...
  children.forEach(child => {
    const layerItem = createLayerItem(child as HTMLElement, level, refreshFn);
    ...
    if (nestedComponents.length > 0) {
      ...
      if (expandToggle) {
        expandToggle.addEventListener('click', e => {
          e.stopPropagation();
          toggleLayerExpansion(expandToggle as HTMLElement, childrenContainer);
        });
      }
...
function createLayerItem(
  ...
  if (visibilityBtn) {
    visibilityBtn.addEventListener('click', e => {
      e.stopPropagation();
      toggleVisibility(element, visibilityBtn as HTMLElement);
    });
  }
