...
jest.mock('../../canvas/Canvas', () => ({
  Canvas: {
    ...
    generateUniqueClass: jest.fn((prefix: string) => `${prefix}-uid`),
...
function makeEditableChild(classList: string[] = []): HTMLElement {
...
describe('ContainerComponent', () => {
  beforeEach(() => {
    ...
    jest.clearAllMocks();
    (Canvas as any).layoutMode = 'absolute';
  });
...
describe('ContainerResizeHandler', () => {
  ...
  let resizersEl: HTMLElement;
  let handler: ContainerResizeHandler;
  ...
  it('mousedown on a resizer should register global mousemove listener', () => {
    ...
    const resizer = resizersEl.querySelector('.bottom-right') as HTMLElement;
...
describe('handleContainerDrop', () => {
  ...
  function makeDragEvent(componentType: string): DragEvent {
...
describe('restoreContainer', () => {
  ...
  it('should show label on mouseenter for editable children', () => {
    ...
    restoreContainer(el, true);
    const label = child.querySelector('.component-label') as HTMLElement;
