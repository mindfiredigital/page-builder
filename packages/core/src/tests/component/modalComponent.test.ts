...
const SAMPLE_ATTRS: ComponentAttribute[] = [
  {
    id: 'attr-1',
    key: 'price',
    title: 'Price',
...
describe('renderForm', () => {
  ...
  it('should render .form-title with attribute title', () => {
    renderForm(container, SAMPLE_ATTRS);
    const titles = Array.from(container.querySelectorAll('.form-title')).map(
      el => el.textContent
    );
  ...
  it('should render .form-key with key and type', () => {
    renderForm(container, SAMPLE_ATTRS);
    const key = container.querySelector('.form-key') as HTMLElement;
  ...
  it('should render .form-display-value span with attribute value', () => {
    renderForm(container, SAMPLE_ATTRS);
    const displayVal = container.querySelector(`#attr-1`) as HTMLElement;
  ...
  it('value container should start collapsed', () => {
    ...
    expect(valueContainer.classList.contains('form-value-collapsed')).toBe(
      true
    );
  });
  ...
  it('clicking header should toggle expansion', () => {
    renderForm(container, SAMPLE_ATTRS);
    const header = container.querySelector('.form-field-header') as HTMLElement;
    header.click();
    const valueContainer = header.nextElementSibling as HTMLElement;
    expect(valueContainer.classList.contains('form-value-collapsed')).toBe(
      false
    );
  });
...
describe('filterAttributes', () => {
  ...
  it('should show fields matching by key', () => {
    ...
    const priceField = container.querySelector(
      '[data-attr-key="price"]'
    ) as HTMLElement;
    expect(priceField.classList.contains('modal-hidden')).toBe(true);
    const qtyField = container.querySelector(
      '[data-attr-key="qty"]'
    ) as HTMLElement;
  ...
  it('should show fields matching by title', () => {
    ...
    const totalField = container.querySelector(
      '[data-attr-key="total"]'
    ) as HTMLElement;
...
describe('handleSave', () => {
  ...
  it('should resolve with selected attribute key/value', () => {
    ...
    handleSave(container, SAMPLE_ATTRS as any, resolve, hide);
  ...
  it('should resolve with empty object when selected attribute has undefined value', () => {
    ...
    const field = cont.querySelector('[data-attr-key="noVal"]') as HTMLElement;
...
describe('ModalComponent', () => {
  ...
  it('show() should remove modal-hidden class', () => {
    ...
    modal.show(SAMPLE_ATTRS as any);
  ...
  it('close button click should resolve promise with null', async () => {
    const modal = new ModalComponent();
    const promise = modal.show(SAMPLE_ATTRS as any);
    const closeBtn = document.getElementById(
      'close-modal-button'
    ) as HTMLButtonElement;
    closeBtn.click();
  ...
  it('save button click should resolve promise with selected field', async () => {
    const modal = new ModalComponent();
    const promise = modal.show(SAMPLE_ATTRS as any);
