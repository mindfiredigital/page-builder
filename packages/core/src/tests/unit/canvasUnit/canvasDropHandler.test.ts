import { CanvasDropHandler } from '../../../canvas/CanvasCore/CanvasDropHandler';
...
describe('CanvasDropHandler', () => {
  ...
  beforeEach(() => {
    ...
    mockDataTransfer = {
      getData: jest.fn(key => {
        if (key === 'component-type') return 'button';
        return '';
      }),
    };
  ...
  it('should call preventDefault on every drop', () => {
    CanvasDropHandler.onDrop(mockEvent as DragEvent);
    expect(mockEvent.preventDefault).toHaveBeenCalled();
  });

  it('should create a component if component-type is valid', () => {
    const mockEl = document.createElement('div');
    (CanvasComponentFactory.createComponent as jest.Mock).mockReturnValue(
      mockEl
    );

    CanvasDropHandler.onDrop(mockEvent as DragEvent);

    expect(CanvasComponentFactory.createComponent).toHaveBeenCalledWith(
      'button',
      ''
    );
  ...
  describe('Printable Mode Constraints', () => {
    it('should prevent drop if position is within margins in printable mode', () => {
      ...
      (
        CanvasSharedState.gridManager.mousePositionAtGridCorner as jest.Mock
      ).mockReturnValue({
        gridX: 10,
        gridY: 10,
      });
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('margin area')
      );
