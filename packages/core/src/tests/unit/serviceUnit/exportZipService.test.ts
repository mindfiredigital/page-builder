import { createExportToZipButton } from '../../../services/ExportZipService';
...
beforeEach(() => {
  ...
  (createZipFile as jest.Mock).mockReturnValue(MOCK_BLOB);
...
// ---------------------------------------------------------------------------
function withAnchorCapture(fn: () => void): HTMLAnchorElement | null {
...
describe('createExportToZipButton', () => {
  ...
  describe('button element', () => {
    ...
    it('has text content "Export to ZIP"', () => {
      expect(createExportToZipButton(HTML, CSS).textContent).toBe(
        'Export to ZIP'
      );
    });
    ...
    it('has the "export-btn" CSS class', () => {
      expect(
        createExportToZipButton(HTML, CSS).classList.contains('export-btn')
      ).toBe(true);
    });
