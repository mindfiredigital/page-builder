 ...
 */
function fireDrop(target: HTMLElement, componentType: string): DragEvent {
  const dt = {
...
describe('MultiColumnContainer', () => {
  ...
  describe('constructor', () => {
    ...
    it('sets each column width proportionally', () => {
      const el = new MultiColumnContainer(4).create();
      const columns = el.querySelectorAll<HTMLElement>('.column');
      columns.forEach(col => {
  ...
  describe('onDrop', () => {
    ...
    beforeEach(() => {
      ...
      (Canvas.createComponent as jest.Mock).mockReturnValue(mockComponent);
      (Canvas.generateUniqueClass as jest.Mock).mockReturnValue('text-c0-abc');
    });
    ...
    it('does nothing when Canvas.createComponent returns null', () => {
      (Canvas.createComponent as jest.Mock).mockReturnValue(null);
      const el = new MultiColumnContainer(2).create();
      document.body.appendChild(el);
      const column = el.querySelector<HTMLElement>('.column')!;
    ...
    it('generates a unique class for the dropped component', () => {
      ...
      expect(Canvas.generateUniqueClass).toHaveBeenCalledWith(
        'text',
        true,
        expect.any(String)
      );
  ...
  describe('restoreColumn()', () => {
    it('calls addControlButtons for each editable-component child', () => {
      ...
      expect(Canvas.controlsManager.addControlButtons).toHaveBeenCalledWith(
        child
      );
    });
    ...
    it('restores image upload for image-component children', () => {
      ...
      expect(ImageComponent.restoreImageUpload).toHaveBeenCalledWith(
        imgChild,
        'test.png',
        null
      );
    ...
    it('calls ContainerComponent.restoreContainer for container-component children', () => {
      ...
      expect(ContainerComponent.restoreContainer).toHaveBeenCalledWith(
        containerChild
      );
    });
