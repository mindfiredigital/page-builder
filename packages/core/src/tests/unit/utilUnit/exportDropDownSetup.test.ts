...
function makeExportBtn(): HTMLElement {
...
function click(target: EventTarget, options: MouseEventInit = {}): void {
...
describe('setupExportDropdown — DOM structure', () => {
  ...
  it('HTML option has text content "HTML"', () => {
    makeExportBtn();
    setupExportDropdown();
    expect(document.getElementById('export-html-btn')!.textContent).toBe(
      'HTML'
    );
  });
  ...
  it('both options carry the class "export-option"', () => {
    ...
    expect(
      document
        ...
        .classList.contains('export-option')
    ).toBe(true);
    expect(
      document
  ...
  it('dropdown does NOT have "visible" class on initial render', () => {
    ...
    setupExportDropdown();
    expect(
      document.querySelector('.export-dropdown')!.classList.contains('visible')
    ).toBe(false);
  });
  ...
  it('HTML option is a child of the dropdown, not the button directly', () => {
    ...
    expect(dropdown.contains(document.getElementById('export-html-btn'))).toBe(
      true
    );
  });
});
...
describe('setupExportDropdown — close on outside click', () => {
  ...
  it('does NOT close the dropdown when clicking inside the export button', () => {
    ...
    const dropdown = btn.querySelector('.export-dropdown') as HTMLElement;
