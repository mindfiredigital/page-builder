...
import { AddPaydataUserProps, FormDataProps, DynamicFormData } from "../../../types/types";
import { paydataUserCreate, paydataUserUpdate, roleList, userList } from "../../../services/services";
...
const initialState: FormDataProps<DynamicFormData> = {
    values: { user_id: "", role_id: "", search_key: "" },
    errors: { name: "", role_id: "" },
    loading: false
};

const Add = ({ open, onClose, data }: AddPaydataUserProps) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const response = data
            ? await paydataUserUpdate(data.id, { role_id: Number(state.values.role_id) })
            : await paydataUserCreate({ role_id: Number(state.values.role_id), user_id: Number(state.values.user_id) });
        ...
        <Dialog open={open} onClose={handleClose} fullWidth={true} maxWidth="xs">
            ...
            <DialogContent sx={{ paddingTop: "10px !important" }}>
                ...
                {!data && <Input
                    ...
                    placeholder="Search User by Username or Code"
                    tabIndex={1}
                />}
